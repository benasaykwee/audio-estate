...
/* ---- 9½. THE ANALYSER TAPS ----
   scTap has existed since round 3 and had ZERO parity coverage — the gate
   only ever contained what someone remembered to add, which is the same
   hole the round-10 audit found in bypass and scListen. outTap arrives
   with coverage rather than acquiring it later.

   What these cases are actually pinning, beyond "the numbers match":
   · the ring INDEXING. Both taps read backwards through a masked circular
     buffer, and an off-by-one in the wrap is invisible in the audio and
     fatal in the display.
   · that scTap is NOT written while bypassed (the engine returns before
     the sidechain runs) while outTap IS. That asymmetry is deliberate and
     therefore has to be pinned, or a future tidy-up will "fix" it.
   · Multi's delegation at bands === 1 against its own ring above that. */
var TAPN = 1024, TAPSTRIDE = 13;
var TAPS = [
  { style: 'fresh', thresh: -30, ratio: 8, scOn: true, scHp: 120, scLp: 8000 },
  { style: 'fresh', thresh: -30, ratio: 8, look: 7, bypass: true },
  { style: 'settling', thresh: -26, ratio: 4, scOn: true, scListen: true },
  { style: 'fresh', thresh: -30, ratio: 8, multi: true },
  { style: 'fresh', thresh: -30, ratio: 8, bands: 2, xover: [300, 3000] },
  { style: 'repose', thresh: -26, ratio: 4, bands: 3, xover: [200, 3000] }
];
var taps = [];
TAPS.forEach(function (o) {
  var s = baseState(o);
  var useMulti = o.multi || s.bands > 1;
  var e4 = (useMulti ? R.createMulti : R.createEngine)(FS);
  e4.setState(s);
  var oL = new Float64Array(N), oR = new Float64Array(N);
  e4.process(srcL, srcR, oL, oR);
  var tsc = new Float64Array(TAPN), tou = new Float64Array(TAPN);
  e4.scTap(tsc); e4.outTap(tou);
  for (var k = 0; k < TAPN; k += TAPSTRIDE) taps.push(tsc[k], tou[k]);
});
...
var out =
  ...
  arr('EXP_SPEC', spec) + '\n' + arr('EXP_R9', r9) + '\n' + arr('EXP_AUD', aud) + '\n' +
  arr('EXP_TAPS', taps) +
  '\n#define EXP_TAPN ' + TAPN + '\n#define EXP_TAPSTRIDE ' + TAPSTRIDE + '\n' +
